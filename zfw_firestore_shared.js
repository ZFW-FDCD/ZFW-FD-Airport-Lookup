...
(function () {
  ...
  function timeoutPromise(ms) {
    return new Promise(function (_, reject) {
      setTimeout(function () { reject(new Error("Firebase connection timed out.")); }, ms);
    });
  }
  ...
  function isNonZfwType(type) {
    ...
    return value === "non_zfw_airports" || value === "non_zfw_airport" || value === "non-zfw-airports" || value === "non_zfw" || value === "non-zfw";
  }

  function isFacilityType(type) {
    return String(type || "").toLowerCase() === "facility_contacts" || String(type || "").toLowerCase() === "facility";
  }

  function recordIsNonZfw(record) {
    if (!record) return false;
    return isNonZfwType(record.data_category) || isNonZfwType(record.category) || String(record.record_type || "").toUpperCase() === "NON_ZFW_AIRPORT";
  }
  ...
  function applyRecord(type, ident, record) {
    ...
    if (isFacilityType(type) || String(record.record_type || "").toUpperCase() === "FACILITY") {
    ...
    if (isNonZfwType(type) || recordIsNonZfw(record)) {
      ...
      if (cleanIdent.length === 4 && cleanIdent.startsWith("K")) cleanIdent = cleanIdent.slice(1);
      const center = String(record.center || "").toUpperCase();
      const cleanRecord = { center: center, name: record.name || record.airport_name || cleanIdent, fdcd: record.fdcd || "" };
    ...
    if (isAirportType(type)) {
      ...
      if (ident.length === 4 && ident.startsWith("K")) records[ident.slice(1)] = clone(record);
      else if (ident.length === 3) records["K" + ident] = clone(record);
      notifyDataChanged();
      return;
    }
    ...
    const fakeK = "K" + ident;
    if (records[fakeK] && String(records[fakeK].record_type || "").toUpperCase() !== "AIRPORT") delete records[fakeK];
    notifyDataChanged();
  }

  function notifyDataChanged() {
    if (window.ZFW_MERGE_NAV_DATA) {
      try { window.ZFW_MERGE_NAV_DATA(); } catch (error) { console.warn("Could not merge shared nav data:", error.message || error); }
    }
    ...
    if (airportInput && airportInput.value) airportInput.dispatchEvent(new Event("input", { bubbles: true }));
    const omicInput = document.getElementById("omicInput");
    if (omicInput && omicInput.value) omicInput.dispatchEvent(new Event("input", { bubbles: true }));
  ...
  async function initFirebase() {
    ...
    try {
      ...
      await Promise.race([authMod.signInAnonymously(auth), timeoutPromise(FIREBASE_TIMEOUT_MS)]);
      initialized = true;
    ...
    initFirebase().then(function (ok) {
      ...
      try {
        ...
        [
          ...
          onSnapshot(collection(db, "zfw_corrections", group.path, "records"), function (snapshot) {
            snapshot.docChanges().forEach(function (change) {
              if (change.type !== "removed") applyRecord(group.type, change.doc.id, change.doc.data());
            });
          }, function (error) { console.warn("Firestore listener stopped:", error.message || error); });
        });
  ...
  async function saveSharedRecord(type, ident, record) {
    ...
    const facility = isFacilityType(type) || String(record && record.record_type || "").toUpperCase() === "FACILITY";
    ...
    try {
      ...
      if (facility) collectionName = "facility_contacts";
      const cleanRecord = clone(record);
      cleanRecord.identifier = ident;
      cleanRecord.data_category = nonZfw ? "non_zfw_airports" : (facility ? "facility_contacts" : collectionName);
